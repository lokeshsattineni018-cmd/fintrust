...
export function TransactionsTable() {
  ...
  return (
    <>
      <div className="bg-white rounded-3xl overflow-hidden glass-card">
        <div className="p-8 pb-4">
          <div className="flex flex-col gap-8">
            <div className="flex justify-between items-end border-b border-zinc-100 pb-8">
               <div className="space-y-1">
                  <h2 className="text-2xl font-bold tracking-tight text-zinc-900 font-display">FinTrust Log</h2>
                  <p className="text-zinc-500 font-medium text-sm">System audit of all financial records</p>
               </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4">
              <div className="relative flex-1 min-w-[280px]">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-4 py-2.5 bg-zinc-100/50 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900/10 focus:bg-white transition-all text-zinc-900 placeholder-zinc-400"
                />
              </div>
              ...
              <div className="flex items-center gap-2">
                 <select
                   ...
                   className="bg-zinc-100/50 px-4 py-2.5 rounded-full text-[13px] text-zinc-900 focus:outline-none cursor-pointer transition-colors hover:bg-zinc-200/50"
                 >
                 ...
                 <select
                   ...
                   className="bg-zinc-100/50 px-4 py-2.5 rounded-full text-[13px] text-zinc-900 focus:outline-none cursor-pointer hidden sm:block transition-colors hover:bg-zinc-200/50"
                 >
        ...
        <div className="overflow-x-auto overflow-y-auto max-h-[65vh] relative custom-scrollbar">
          {filtered.length === 0 ? (
            ...
            <table className="w-full min-w-[700px] text-left border-separate border-spacing-0">
              <thead>
                <tr className="bg-zinc-50 text-[10px] uppercase tracking-[0.2em] text-zinc-400 font-bold border-t border-zinc-200/60">
                  <th className="sticky top-0 z-20 bg-zinc-50 py-4 px-8 border-b border-zinc-200/50 group cursor-pointer hover:bg-zinc-100 transition-colors" onClick={() => toggleSort('description')}>
                    <div className="flex items-center gap-2">
                      Transaction Description
                      <SortHeaderIcon field="description" sortField={sortField} sortDirection={sortDirection} />
                    </div>
                  </th>
                  <th className="sticky top-0 z-20 bg-zinc-50 py-4 px-8 border-b border-zinc-200/50 group cursor-pointer hover:bg-zinc-100 transition-colors" onClick={() => toggleSort('category')}>
                    <div className="flex items-center gap-2">
                       Classification
                      <SortHeaderIcon field="category" sortField={sortField} sortDirection={sortDirection} />
                    </div>
                  </th>
                  <th className="sticky top-0 z-20 bg-zinc-50 py-4 px-8 border-b border-zinc-200/50 group cursor-pointer hover:bg-zinc-100 transition-colors hidden sm:table-cell" onClick={() => toggleSort('date')}>
                    <div className="flex items-center gap-2">
                       Posting Date <SortHeaderIcon field="date" sortField={sortField} sortDirection={sortDirection} />
                    </div>
                  </th>
                  <th className="sticky top-0 z-20 bg-zinc-50 py-4 px-8 border-b border-zinc-200/50 group cursor-pointer hover:bg-zinc-100 transition-colors text-right" onClick={() => toggleSort('amount')}>
                    <div className="flex items-center justify-end gap-2">
                       Net Value <SortHeaderIcon field="amount" sortField={sortField} sortDirection={sortDirection} />
                    </div>
                  </th>
                  {role === 'admin' && <th className="sticky top-0 z-20 bg-zinc-50 py-4 px-8 border-b border-zinc-200/50 text-center w-28">Actions</th>}
                </tr>
              </thead>
              <tbody>
                        ...
                        <td className="py-5 px-8 border-b border-zinc-50">
                          <div className="flex items-center gap-4">
                            <div className={`w-11 h-11 rounded-2xl flex items-center justify-center transition-all duration-300 ${
                              isIncome ? 'bg-emerald-50 text-emerald-600' : 'bg-zinc-100 text-zinc-900 border border-zinc-200/40 shadow-sm'
                            }`}>
                              {isIncome ? <ArrowUpRight size={18} strokeWidth={2.5} /> : <ArrowDownLeft size={18} strokeWidth={2.5} />}
                            </div>
                            <span className="font-bold text-sm tracking-tight text-zinc-900 font-display">{tx.description}</span>
                          </div>
                        </td>
                        <td className="py-5 px-8 border-b border-zinc-50">
                           <div className="flex items-center gap-3">
                              <div className="w-8 h-8 bg-zinc-50 rounded-xl flex items-center justify-center text-zinc-400 group-hover:text-zinc-600 transition-colors border border-transparent group-hover:border-zinc-200">
                                 <Icon size={14} strokeWidth={2.5} />
                              </div>
                              <span className="text-[13px] font-bold text-zinc-500 uppercase tracking-widest">{tx.category}</span>
                           </div>
                        </td>
                        <td className="py-5 px-8 border-b border-zinc-50 hidden sm:table-cell text-zinc-400 text-[13px] font-semibold font-numeric tracking-tight">
                          {tx.date}
                        </td>
                        <td className={`py-5 px-8 border-b border-zinc-50 text-right font-bold text-sm font-numeric tracking-tighter ${
